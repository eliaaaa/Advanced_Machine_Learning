...
lambda = 1e-3;
w = zeros(N,1); 
cost_by_iteration_sgd100 = [];
time_by_iteration_sgd100 = [];
...
while(i<20000)
    ...
    gradient = loss_gradient(TrainingY(sample), Gram_matrix(sample,:), w, lambda);
    
    cost_by_iteration_sgd100(i) = loss_now;
    time_by_iteration_sgd100(i) = toc;
...
p = plot(time_by_iteration_sgd100, cost_by_iteration_sgd100, 'b-', 'LineWidth', 1); 
xlabel('Time (s)'); 
ylabel(sprintf('Cost'));
title ( 'Stochastic Gradient Descent p = 100') ;
saveas(p, 'sgd100.png', 'png');
